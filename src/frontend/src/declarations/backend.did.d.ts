/* eslint-disable */

// @ts-nocheck

// This file was automatically generated by @caffeineai/bindgen@0.3.1.
// You should NOT make any changes in this file as it will be overwritten.
// Additionally, you should also exclude this file from your linter and/or formatter to prevent it from being checked or modified.

import type { ActorMethod } from '@icp-sdk/core/agent';
import type { IDL } from '@icp-sdk/core/candid';
import type { Principal } from '@icp-sdk/core/principal';

export type AiDifficulty = { 'normal' : null } |
  { 'easy' : null } |
  { 'hard' : null };
export interface Cell { 'value' : Value, 'name' : string }
export type Error = { 'FrontendOriginsNotConfigured' : null } |
  {
    'MixedSsoSources' : {
      'otherKeys' : Array<string>,
      'ssoKeys' : Array<string>,
    }
  } |
  { 'Stale' : { 'ageNs' : bigint } } |
  { 'MalformedCandid' : null } |
  { 'AmbiguousAttribute' : { 'field' : string, 'sources' : Array<string> } } |
  { 'NoAttributes' : null } |
  { 'UnknownNonce' : null } |
  { 'UntrustedSsoSource' : { 'domain' : string } } |
  { 'MissingField' : string } |
  { 'FrontendOriginMismatch' : { 'got' : string, 'expected' : Array<string> } };
export type Faction = { 'vikings' : null } |
  { 'mongols' : null } |
  { 'romans' : null };
export interface FactionStats {
  'totalMatches' : bigint,
  'wins' : bigint,
  'losses' : bigint,
  'faction' : Faction,
}
export type MapSize = { 'large' : null } |
  { 'small' : null } |
  { 'medium' : null };
export type MatchId = bigint;
export interface MatchRecord {
  'id' : MatchId,
  'result' : MatchResult,
  'mapSize' : MapSize,
  'difficulty' : AiDifficulty,
  'recordedAt' : Timestamp,
  'durationSeconds' : bigint,
  'faction' : Faction,
}
export type MatchResult = { 'defeat' : null } |
  { 'victory' : null };
export interface MatchStats {
  'totalMatches' : bigint,
  'byFaction' : Array<FactionStats>,
  'wins' : bigint,
  'losses' : bigint,
  'totalDurationSeconds' : bigint,
}
export interface PlayerSettings {
  'preferredFaction' : Faction,
  'preferredDifficulty' : AiDifficulty,
  'lastMapSize' : MapSize,
}
export interface Result { 'hasMore' : boolean, 'rows' : Array<Array<Cell>> }
export type Result__1 = { 'ok' : null } |
  { 'err' : Error };
export type Timestamp = bigint;
export type UserRole = { 'admin' : null } |
  { 'user' : null } |
  { 'guest' : null };
export type Value = { 'int' : bigint } |
  { 'nat' : bigint } |
  { 'float' : number } |
  { 'bool' : boolean } |
  { 'null' : null } |
  { 'text' : string };
export interface _SERVICE {
  '_initialize_access_control' : ActorMethod<[], undefined>,
  '_internet_identity_sign_in_finish' : ActorMethod<[], Result__1>,
  '_internet_identity_sign_in_start' : ActorMethod<[], Uint8Array>,
  'assignCallerUserRole' : ActorMethod<[Principal, UserRole], undefined>,
  'execute' : ActorMethod<[string], Result>,
  /**
   * / Returns the public API documentation as static Markdown.
   */
  'getApiDoc' : ActorMethod<[], string>,
  'getCallerUserRole' : ActorMethod<[], UserRole>,
  /**
   * / Returns aggregate win/loss and per-faction statistics for the caller.
   */
  'getMatchStats' : ActorMethod<[], MatchStats>,
  /**
   * / Reads the caller's saved settings.
   */
  'getSettings' : ActorMethod<[], [] | [PlayerSettings]>,
  'isCallerAdmin' : ActorMethod<[], boolean>,
  /**
   * / Lists the caller's most recent matches, newest first.
   */
  'listRecentMatches' : ActorMethod<[bigint], Array<MatchRecord>>,
  /**
   * / Records a completed skirmish for the signed-in caller.
   */
  'recordMatch' : ActorMethod<
    [Faction, MapSize, AiDifficulty, MatchResult, bigint],
    MatchId
  >,
  /**
   * / Saves the caller's settings and returns the stored value.
   */
  'saveSettings' : ActorMethod<[PlayerSettings], PlayerSettings>,
  'schema' : ActorMethod<[], string>,
}
export declare const idlService: IDL.ServiceClass;
export declare const idlInitArgs: IDL.Type[];
export declare const idlFactory: IDL.InterfaceFactory;
export declare const init: (args: { IDL: typeof IDL }) => IDL.Type[];